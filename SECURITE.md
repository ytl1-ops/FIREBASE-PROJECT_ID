...
| R3 | La transcription *sur l'appareil* télécharge les modèles depuis Hugging Face (flux sortant vers un tiers, possiblement bloqué par le proxy MSC). | Faible (aucune donnée de réunion) | En entreprise, privilégier la transcription par « Mon API » ; sinon, autoriser `huggingface.co` ou héberger un miroir interne des modèles. |
| R4 | Les réunions sont stockées **en clair dans le navigateur** du terminal ; une perte ou un vol de terminal non chiffré les expose. | Élevé sur terminal non géré | Terminaux gérés par **MDM** avec chiffrement et verrouillage ; usage sur terminaux personnels à proscrire pour les réunions sensibles ; purge régulière (Informations → Supprimer). |
